...
{
    [OmniSharpHandler(OmniSharpEndpoints.MembersTree, LanguageNames.VisualBasic)]
    public class MembersAsTreeService : IRequestHandler<MembersTreeRequest, FileMemberTree>
    {
        ...
        public async Task<FileMemberTree> Handle(MembersTreeRequest request)
        {
            return new FileMemberTree()
            {
                ...
                TopLevelTypeDefinitions = await VisualBasicStructureComputer.Compute(await _workspace.GetDocumentsFromFullProjectModelAsync(request.FileName), _discovers)
            };
        }
    }
}
