...
{
    [OmniSharpHandler(OmniSharpEndpoints.MembersFlat, LanguageNames.VisualBasic)]
    public class MembersAsFlatService : IRequestHandler<MembersFlatRequest, IEnumerable<QuickFix>>
    {
        ...
        {
            var stack = new List<FileMemberElement>(await VisualBasicStructureComputer.Compute(_workspace.GetDocuments(request.FileName)));
